...
@section('headerContent')<h3 class="font-bold pl-2">Edit Subject: {{ $subject->name }}</h3>@endsection

@section('content')
    <form action="{{ route('admin.subjects.update', $subject->id) }}" method="POST" class="space-y-6 max-w-lg mx-auto">
        @csrf
        @method('PUT')
        <div>
            ...
            <input type="text" name="name" id="name" value="{{ old('name', $subject->name) }}" required class="mt-1 block w-full admin-input @error('name') border-red-500 @enderror">
        ...
        <div>
            ...
            <input type="number" name="exam_duration_minutes" id="exam_duration_minutes" value="{{ old('exam_duration_minutes', $subject->exam_duration_minutes) }}" required min="1" class="mt-1 block w-full admin-input @error('exam_duration_minutes') border-red-500 @enderror">
