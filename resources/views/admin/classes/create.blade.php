...
@section('headerContent')<h3 class="font-bold pl-2">Add New Class</h3>@endsection

@section('content')
    <form action="{{ route('admin.classes.store') }}" method="POST" class="space-y-6 max-w-lg mx-auto">
        @csrf
        <div>
        ...
        <div>
            ...
            <select name="level_group" id="level_group" required class="mt-1 block w-full admin-input @error('level_group') border-red-500 @enderror">
                ...
                @foreach($levelGroups as $group)
                    <option value="{{ $group }}" {{ old('level_group') == $group ? 'selected' : '' }}>{{ $group }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3" class="mt-1 block w-full admin-input @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
            @error('description') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
        </div>
        <div class="flex justify-end space-x-3 pt-4">
            <a href="{{ route('admin.classes.index') }}" class="admin-btn-secondary">Cancel</a>
            <button type="submit" class="admin-btn-primary">Create Class</button>
        </div>
    </form>
@endsection
